...
Recently, I encountered a challenge while working on an iOS project that required a specific functionality that was too complicated to implement by myself. My search led me to a Java library that had the exact features I needed. However, there was one major roadblock: it was only available in Java. Initially, I attempted to find a native alternative, readily available as a third-party dependency that I could integrate into my iOS project via SPM, CocoaPods, etc. Unfortunately, none of the available options matched the functionality of the Java library. As I researched methods for integrating Java into iOS, I discovered that the majority of the existing solutions were outdated, poorly documented, and lacked proper support, rendering them ineffective. Just when I was about to give up, I stumbled upon MobiVM (formerly known as RoboVM). This tool promised the ability to run Java code on iOS devices. This blog post describes my journey of using MobiVM to write a Java wrapper and convert it into an Xcode framework, making it accessible for iOS.
